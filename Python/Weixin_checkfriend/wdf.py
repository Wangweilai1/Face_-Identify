...
def main():
    ...
    for i in range(0, group_num):
        ...
        for i in range(DeletedCount):
            if d[DeletedList[i]][1] != '':
                print(str(d[DeletedList[i]][0]) + '(%s)' %str(d[DeletedList[i]][1]))
            else:
    ...
    for r in result:
        if d[r][1] != '':
            resultNames.append(str(d[r][0]) + '(%s)' % str(d[r][1]))
        else:
            resultNames.append(str(d[r][0]))
...
if __name__ == '__main__':
    ...
    print('                     神不知鬼不觉的查找自己被谁删除了微信好友')
    print('                                               版权所有：WangWeiLai')
    print('开始')
    main()
    print('结束')
